...
import com.hjsoft.guestbooktaxi.fragments.CancelBookedRideFragment;
 ...
 */
public class MyRidesActivity extends AppCompatActivity implements RecyclerAdapter.AdapterCallback {
    ...
    private void selectItem(int position) {
        ...
        switch (position) {
            case 0:
                ...
                break;
            case 1:
                Intent o=new Intent(this,MyRidesActivity.class);
                startActivity(o);
                finish();
    ...
    public void onMethodCallback(final int position, ArrayList<FormattedAllRidesData> data) {
        ...
        {
            //check if for both travel types SpecificRideOngoingFragment can be used ?
            /*
            if(item.getTravelType().equals("outstation")) //||item.getBookingType().equals("TeleBook")
            {
                Fragment fragmnt = new SpecificRideOngoingOtherFragment();
                fragmnt.setArguments(args);

                FragmentManager fragmentManager = getSupportFragmentManager();
                FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                fragmentTransaction.add(R.id.content_frame, fragmnt, "specific_ride_ongoing_other");
                ...
                FragmentManager fragmentManager = getSupportFragmentManager();
                FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                fragmentTransaction.add(R.id.content_frame, fragmnt, "specific_ride_ongoing");
                fragmentTransaction.addToBackStack(null);
                fragmentTransaction.commit();
            //}
        }
            ...
            else if(status.equals("BOOKED")||status.equals(""))
            {
                Fragment frag = new CancelBookedRideFragment();
                frag.setArguments(args);

                FragmentManager fragmentManager = getSupportFragmentManager();
                FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
                fragmentTransaction.add(R.id.content_frame, frag, "cancel_booked_ride");
                fragmentTransaction.addToBackStack(null);
                fragmentTransaction.commit();
            }
            else {
